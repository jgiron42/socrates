...
# (this sometimes happens with philo_bonus)
...
def socrates(bin_path, bonus):
    ...
    if os.path.isfile(f"{bin_path}/philo/philo"):
        print(f"\n{bcolors.OKBLUE}---------- PHILO----------{bcolors.ENDC}\n")
        test_program(f"{bin_path}/philo/philo")
    if os.path.isfile(f"{bin_path}/philo_bonus/philo_bonus") and (bonus):
...
if __name__ == "__main__":
    ...
    try:
        exit(socrates(args.path, args.bonus))
